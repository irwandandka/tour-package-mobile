...
import { ScrollView, Text, Image, TouchableOpacity, View, ActivityIndicator, Linking } from "react-native";
            ...
            <ScrollView>
                <View style={styles.container}>
                    ...
                    <View style={styles.groupPanel}>
                        {rooms.map((room, index) => (
                            <View key={index} style={styles.groupRoomDetail}>
                                ...
                                {room.adult > 0 && <Text style={styles.passengerLabel}>{room.adult} Adult x IDR {(Number(room.priceAdult) || 0).toFixed(2)}</Text>}
                                {room.child > 0 && <Text style={styles.passengerLabel}>{room.child} Child x IDR {(Number(room.priceChild) || 0).toFixed(2)}</Text>}
                                {room.infant > 0 && <Text style={styles.passengerLabel}>{room.infant} Infant x IDR {(Number(room.priceInfant) || 0).toFixed(2)}</Text>}
                                {room.senior > 0 && <Text style={styles.passengerLabel}>{room.senior} Senior x IDR {(Number(room.priceSenior) || 0).toFixed(2)}</Text>}
                            </View>
                        ))}
                        <View style={{ height: 1, backgroundColor: '#eee', marginVertical: 10 }} />
                        <View style={styles.groupAmountDetail}>
                            <Text style={styles.amountDetail}>Subtotal Amount</Text>
                            <Text style={styles.amountDetail}>IDR {(Number(transaction?.total_amount) || 0).toFixed(2)}</Text>
                        </View>
                        <View style={styles.groupAmountDetail}>
                    ...
                    <View style={styles.groupPanel}>
                        ...
                        <TouchableOpacity 
                            style={[styles.buttonPay, (isLoading || !selectedPayment) && { backgroundColor: '#ccc' }]}
